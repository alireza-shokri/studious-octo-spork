...
function change_theme_body(status){
    if(status=='dark'){
        ...
        $.documentElement.style.setProperty('--color_menu','rgba(255, 255, 255, 0.835)');
        $.documentElement.style.setProperty('--background_sect1','rgb(9, 13, 56)');
        $.documentElement.style.setProperty('--color_sect1_desc','wheat');
        $.documentElement.style.setProperty('--color_sect1_communication','goldenrod');
    }
    else{
        $.documentElement.style.setProperty('--color_body',' rgba(162, 178, 179,0.3)');
        $.documentElement.style.setProperty('--color_black_desc_skill','black');
        $.documentElement.style.setProperty('--color_menu','');
        $.documentElement.style.setProperty('--background_sect1','rgb(177, 178, 211)');
        $.documentElement.style.setProperty('--color_sect1_desc','rgb(14, 13, 12)');
        $.documentElement.style.setProperty('--color_sect1_communication','rgb(16, 14, 139)');
    }
}
...
function appear_menu(){ 
    ...
    if(vaz_menu=='2'){
        if(all_style_menu.right!='-250px')
            menu.style.right='-250px';
        else  menu.style.right='16px';
    }
    else if(vaz_menu=='1'){
        if( all_style_menu.top!='-80px')
            menu.style.top='-80px';
        else  menu.style.top='16px';
    }
}
...
function interval_fanc(){
     secinterval=setInterval(() => {
        if(conter < sect3_all_box.length - (Math.floor(sect3_p_box.offsetWidth / sect3_all_box[0].offsetWidth))&& !sect3_vaz)
            sect3_work_right()
        else{
            sect3_work_left()
            sect3_vaz=true;
...
function sect3_work_left(){
    if(conter>0){
        ...
        if(conter==0)
            sect3_left_2.style.color='wheat';
...
function sect3_work_right(){
    if(conter < sect3_all_box.length - (Math.floor(sect3_p_box.offsetWidth / sect3_all_box[0].offsetWidth ) )){
        go_box(+(jahat))
        conter++;
...
sect3_p_box.addEventListener('touchstart',function(event){sect3_megdar_touch=event.touches[0].clientX;})
...
window.addEventListener('scroll', function() {
   let posithon_sect3=$.querySelector('.section_3').getBoundingClientRect(); 
    if (posithon_sect3.top <= 0 && posithon_sect3.bottom >= 0 && $.documentElement.style.getPropertyValue('--color_body')==('rgba(162, 178, 179,0.3)')){
        ...
        if($.documentElement.style.getPropertyValue('--color_menu_link')!=('rgb(5, 3, 0)')){
